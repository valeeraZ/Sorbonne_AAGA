...
from Project.remy import RemyTree
import pydot


class RemyTree_2(RemyTree):
    def __init__(self, N):
        super().__init__(N)
        self.root = 0
...
def test_covering_2(n):
    trees = gen_all_trees_2(n)
    trees_compressed = list(map(RemyTree_2.compress, trees))
...
def test_small_coverings_2():
    return all([test_covering_2(i) for i in range(7)])
